...
namespace upa {
    ...
    void StructuredMesh::getElemDOFs(int e, int *dofs) {
        if (e > _nElems) throw std::runtime_error("femplas_StrcturedMesh: Element ID out of range.");
    ...
    void StructuredMesh::getNodeCoords(int d, double *dofCoords) {
        if (d > _nNodes) throw std::runtime_error("femplas_StrcturedMesh: Node ID out of range.");
    ...
    void StructuredMesh::produceCartesian(int dim, int nSide, ElemType type) {
        ...
        switch (_elemType) {
            case ElemType::Line:
                if (dim != 1) throw std::runtime_error("femplas_StrcturedMesh: ElemType::Line needs dim = 1");
                _nNbors = 2;
            ...
            case ElemType::Square:
                if (dim != 2) throw std::runtime_error("femplas_StrcturedMesh: ElemType::Square needs dim = 2");
                _nNbors = 4;
                connect = new int[_nElems*_nNbors];
                ...
                for (int i = 0; i < _nElems; ++i) {
                    connect[i * _nNbors + 0] = i;
                    connect[i * _nNbors + 1] = i + 1;
                    connect[i * _nNbors + 2] = i + nSide + 1;
                    connect[i * _nNbors + 3] = i + nSide + 2;
                }
                break;

            default :
                throw std::runtime_error("femplas_StrcturedMesh: Element Type not recognised.");
