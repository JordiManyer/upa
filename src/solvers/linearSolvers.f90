...
! #####################################################
    ...
    SUBROUTINE LU( n , A , P)
        ...
        DO k = 1 , n ! Loop over columns
            ...
            IF (k /= pivot) THEN ! Do we need to swap rows?
                ! swap rows
                DO j = 1 , n
                    auxElem = A(k , j)
                    A(k , j) = A(pivot , j)
    ...
    ! Solves Ax = b by using it's LU decomposition with partial pivoting.
    ! Ax = b -> PAx = Pb -> LUx = Pb -> Ly = Pb && Ux = y
    SUBROUTINE solveLU(n , LU , P , b , x)
        implicit NONE
        INTEGER n, P(:), i, j
        REAL LU(:,:), b(:), x(:)
        REAL sum, y(n)

        ! First, we solve L·y = P(b) (Lower triangular system, Lii = 1.0 implicitly)
        DO i = 1 , n
            sum = 0.0
            DO j = 1 , i-1
                sum = sum + LU(i,j)*y(j)
